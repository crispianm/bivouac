    ...
    def bool_features(self):
        ...
        for feat in self.features:
            if feat.shape_type == "Point":
                poly_point = np.logical_and(
                    self.grid[0] == feat.shape[0][0], self.grid[1] == feat.shape[0][1]
                )
...
class heatmap_layer:
    ...
    def make_grid(self):
        ...
        """
        x = np.outer(
            np.linspace(self.SE[0], self.NW[0], 1 + self.n_points[0]),
            np.ones(1 + self.n_points[0]),
        )
        y = np.outer(
            np.linspace(self.SE[1], self.NW[1], 1 + self.n_points[0]),
            np.ones(1 + self.n_points[0]),
        ).T
        z = np.zeros(x.shape)
        self.grid = [x, y, z]
    ...
    def get_bad_features(self, grid, distance, layers):
        ...
        for unique_feature in set(self.bad_features).intersection(set(layers.keys())):
            layer1 = map_layer(
                grid, unique_feature, 1, distance, layers[unique_feature], 1
            )
            layer1.bool_features()
    ...
    def get_good_features(self, grid, distance, layers):
        ...
        for unique_feature in tqdm(
            set(self.preferences.keys()).intersection(set(layers.keys()))
        ):
            layer1 = map_layer(
                grid,
                unique_feature,
    ...
    def make_layers(self, file_name, distance=100):
        """
    ...
    def plot_2D_heatmap(self):
        ...
        """
        fig, ax = plt.subplots()
        heatmap_plot = ax.pcolor(
            self.grid[0], self.grid[1], self.grid[2], cmap="inferno", shading="auto"
        )
        plt.xlabel("X")
        plt.ylabel("Y")
